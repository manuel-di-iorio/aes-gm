 ...
 * @license MIT
 * 
 * Converted to use global functions instead of struct
 */

// --- AES Constants (Global Variables) ---
global.__aes_sbox = [
...
function __aes_get_key_bytes(key) {
...
function __aes_key_expansion(key) {
    ...
    while (i < 176) {
        ...
        if (i % 16 == 0) {
            ...
            // SubWord
            for(var t=0; t<4; t++) temp[t] = global.__aes_sbox[temp[t]];

            // Rcon
            temp[0] = temp[0] ^ global.__aes_rcon[i / 16];
        }
...
function __aes_sub_bytes(state) {
    for (var i = 0; i < 16; i++) {
        state[i] = global.__aes_sbox[state[i]];
    }
}

function __aes_shift_rows(state) {
...
function __aes_gmul2(x) {
    return ((x << 1) & 0xFF) ^ ((x & 0x80) ? 0x1b : 0x00);
}

function __aes_gmul3(x) {
    return __aes_gmul2(x) ^ x;
}

function __aes_mix_columns(state) {
    var temp = array_create(16);
    array_copy(temp, 0, state, 0, 16);
    ...
    for (var c = 0; c < 4; c++) {
        ...
        state[i]   = __aes_gmul2(s0) ^ __aes_gmul3(s1) ^ s2 ^ s3;
        state[i+1] = s0 ^ __aes_gmul2(s1) ^ __aes_gmul3(s2) ^ s3;
        state[i+2] = s0 ^ s1 ^ __aes_gmul2(s2) ^ __aes_gmul3(s3);
        state[i+3] = __aes_gmul3(s0) ^ s1 ^ s2 ^ __aes_gmul2(s3);
    }
}

function __aes_add_round_key(state, w, rnd) {
    for (var i = 0; i < 16; i++) {
        state[i] = state[i] ^ w[rnd * 16 + i];
    }
}

function __aes_cipher(block, w) {
    var state = array_create(16);
    array_copy(state, 0, block, 0, 16);

    __aes_add_round_key(state, w, 0);

    for (var rnd = 1; rnd < 10; rnd++) {
        __aes_sub_bytes(state);
        __aes_shift_rows(state);
        __aes_mix_columns(state);
        __aes_add_round_key(state, w, rnd);
    }

    __aes_sub_bytes(state);
    __aes_shift_rows(state);
    __aes_add_round_key(state, w, 10);

    return state;
}

// --- Inverse Functions (Internal) ---

function __aes_inv_sub_bytes(state) {
    for (var i = 0; i < 16; i++) {
        state[i] = global.__aes_rsbox[state[i]];
    }
}

function __aes_inv_shift_rows(state) {
    var temp = array_create(16);
    array_copy(temp, 0, state, 0, 16);
...
function __aes_gmul9(x) { return __aes_gmul2(__aes_gmul2(__aes_gmul2(x))) ^ x; }
function __aes_gmul11(x) { return __aes_gmul2(__aes_gmul2(__aes_gmul2(x)) ^ x) ^ x; }
function __aes_gmul13(x) { return __aes_gmul2(__aes_gmul2(__aes_gmul2(x) ^ x)) ^ x; }
function __aes_gmul14(x) { return __aes_gmul2(__aes_gmul2(__aes_gmul2(x) ^ x) ^ x); }

function __aes_inv_mix_columns(state) {
    var temp = array_create(16);
    array_copy(temp, 0, state, 0, 16);
    ...
    for (var c = 0; c < 4; c++) {
        ...
        state[i]   = __aes_gmul14(s0) ^ __aes_gmul11(s1) ^ __aes_gmul13(s2) ^ __aes_gmul9(s3);
        state[i+1] = __aes_gmul9(s0) ^ __aes_gmul14(s1) ^ __aes_gmul11(s2) ^ __aes_gmul13(s3);
        state[i+2] = __aes_gmul13(s0) ^ __aes_gmul9(s1) ^ __aes_gmul14(s2) ^ __aes_gmul11(s3);
        state[i+3] = __aes_gmul11(s0) ^ __aes_gmul13(s1) ^ __aes_gmul9(s2) ^ __aes_gmul14(s3);
    }
}

function __aes_inv_cipher(block, w) {
    var state = array_create(16);
    array_copy(state, 0, block, 0, 16);
    ...
    for (var rnd = 9; rnd > 0; rnd--) {
        __aes_inv_shift_rows(state);
        __aes_inv_sub_bytes(state);
        __aes_add_round_key(state, w, rnd);
        __aes_inv_mix_columns(state);
    }
...
function aes_encrypt(buffer, key) {
    var keyBytes = __aes_get_key_bytes(key);
    var w = __aes_key_expansion(keyBytes);
    ...
    for (var offset = 0; offset < size; offset += 16) {
        ...
        var encryptedBlock = __aes_cipher(block, w);
 ...
 */
function aes_decrypt(buffer, key) {
    var keyBytes = __aes_get_key_bytes(key);
    var w = __aes_key_expansion(keyBytes);
    ...
    for (var offset = 16; offset < size; offset += 16) {
        ...
        var decryptedBlock = __aes_inv_cipher(currentBlock, w);
 ...
 */
function aes_decrypt_string(str, key) {
    var encBuf = buffer_base64_decode(str);
    var decBuf = aes_decrypt(encBuf, key);
